...
import { VetsDataLg as InitialVetsData } from '../components/Laura/VetsDataLg';
import SidebarLg from '../components/Laura/SidebarLg';
import HeaderAdmiGeneralLg from '../components/Laura/HeaderAdmiGeneralLg';
import FilterLg from '../components/Laura/FilterLg';
import TableVetsLg from '../components/Laura/TableVetsLg';
...
const AdmiGeneralVetLg = () => {
  ...
  const handleDeleteVet = (vetToDelete) => {
    console.log("Eliminar veterinaria:", vetToDelete.name);
    setVetsData(prevData => prevData.filter(vet => vet.id !== vetToDelete.id));
    setSelectedVet(null); // Ocultar la tarjeta de detalle después de eliminar
    ...
    <div className="containerAg-Lg">
      ...
      {selectedVet && !showAddVetForm && !showProfileCreated && (
        <DetailCardLg
          ...
          // No pasar onActivateClick, onDeactivateClick, onDeleteClick aquí
          titleField="name"
          subtitleField="nit"
        />
      )}
